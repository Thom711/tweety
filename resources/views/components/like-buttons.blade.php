<div class="flex">
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        <div class="flex items-center {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-400' }} mr-3">
            ...
            <button type="submit">
                <span class="text-xs text-gray-400">{{ $tweet->likes ?: 0}}</span>
            </button>
        </div>
    </form>
    ...
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        ...
        <div class="flex items-center {{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-400' }}">
            <svg viewBox="0 0 20 20" class="mr-1 w-3">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
            ...
            <button>
                <span class="text-xs text-gray-400">{{ $tweet->dislikes ?: 0 }}</span>
            </button>
        </div>
    </form>
</div>
